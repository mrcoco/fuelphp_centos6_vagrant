...
class myinputfilters_Text extends TestCase
{
    public function test_check_encoding_SJIS文字列を検証すると例外が発生()
    {
        $this->setExpectedException(
            'Exception_HttpInvalidInputException', 'Invalid input data'
        );

        $input = mb_convert_encoding('SJISの文字列です', 'SJIS');
        $test = Inputfilter_Myinputfilters::check_encoding($input);
    }
    ...
    public function test_check_control_制御文字を含む文字列を検証すると例外が発生($data)
    {
        ...
        $input = $data;
        $test = Inputfilter_Myinputfilters::check_control($input);
    }
