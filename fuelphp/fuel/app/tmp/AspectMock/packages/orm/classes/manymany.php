...
class ManyMany extends Relation
{
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($from, $name, $config), false)) !== __AM_CONTINUE__) return $__am_res; 
		$this->name        = $name;
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($from, $conditions), false)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($alias_from, $rel_name, $alias_to_nr, $conditions), false)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($model_from, $models_to, $original_model_ids, $parent_saved, $cascade), false)) !== __AM_CONTINUE__) return $__am_res; 
		if ( ! $parent_saved)
		{
			return;
		}
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($model_from, $models_to, $parent_deleted, $cascade), false)) !== __AM_CONTINUE__) return $__am_res; 
		if ( ! $parent_deleted)
		{
			return;
		}
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($model_from), false)) !== __AM_CONTINUE__) return $__am_res; 
		// Delete all relationship entries for the model_from
		$query = \DB::delete($this->table_through);
		reset($this->key_from);
		foreach ($this->key_through_from as $key)
